...
function kernel_package_callback_linux_headers() {
        ...
        # Target directories
        local headers_target_dir="${package_directory}/usr/src/linux-headers-${kernel_version_family}"
        local modules_target_dir="${package_directory}/lib/modules/${kernel_version_family}"

        mkdir -p "${headers_target_dir}" "${modules_target_dir}"
        run_host_command_logged ln -v -s "/usr/src/linux-headers-${kernel_version_family}" "${modules_target_dir}/build"
        run_host_command_logged cp -vp "${kernel_work_dir}/.config" "${headers_target_dir}/.config"
        ...
        if [[ "${SRC_ARCH}" != "${KERNEL_SRC_ARCH}" ]]; then
                display_alert "WARNING: KERNEL_SRC_ARCH and SRC_ARCH mismatch" \
                        "KERNEL_SRC_ARCH=${ARCHITECTURE} SRC_ARCH=${SRC_ARCH}" "wrn"
        fi
        ...
        (
                ...
                # Optional BSP headers
                if [[ -d bsp/include ]]; then
                        find bsp/include -type f
                else
                        echo "Skipping BSP headers: bsp/include not present" >&2
                fi

                # Security subsystem headers
                ...
                # Module.symvers, include/, scripts/, and optional BSP include
                if [[ -d bsp/include ]]; then
                ...
                # Tools
                find tools -type f
                find arch/x86/lib/insn.c

                if is_enabled CONFIG_GCC_PLUGINS; then
                        find scripts/gcc-plugins -name gcc-common.h
                fi

        ) > "${temp_file_list}"

        # Copy files into headers directory
        tar -c -f - -C "${kernel_work_dir}" -T "${temp_file_list}" | tar -xf - -C "${headers_target_dir}"

        # Hackfixes for kernel tools
        ...
        if [[ ! -f "${headers_target_dir}/tools/pci/Makefile" ]] && [[ "${KERNEL_MAJOR_MINOR}" == "6.14" ]]; then
                ...
                mkdir -p "${headers_target_dir}/tools/pci"
        ...
        mkdir -p "${headers_target_dir}/tools/counter/include/linux"

        # Clean scripts and tools
        ...
        # module.lds fix
        if [[ -f "${kernel_work_dir}/scripts/module.lds" ]]; then
                cp -v "${kernel_work_dir}/scripts/module.lds" "${headers_target_dir}/scripts/module.lds"
        fi
        ...
        if [[ "${DEBUG}" == "yes" ]]; then
                ...
                (
                        ...
                        find . -type f | grep -v -e "include/config/" -e "\.h$" -e ".c$" -e "Makefile$" -e "Kconfig$" -e "Kbuild$" -e "\.cocci$" \
                                | xargs file | grep -v -e "ASCII" -e "script text" -e "empty" -e "Unicode text" -e "symbolic link" -e "CSV text" -e "SAS 7+" || true
                )
        fi
...
Use this hook to do that, by modifying the file in place in ${headers_target_dir}.
...
Description: Armbian Linux $BRANCH headers ${kernel_version_family}
 Kernel header files for ${kernel_version_family}
CONTROL_FILE
